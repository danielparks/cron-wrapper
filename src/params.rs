...
pub struct Params {
    ...
    /// What signal to use to kill the child process when a timeout or other
    /// error occurs.
    ///
    /// This may be set to "none" to not kill the child process.
    #[clap(long, default_value = "SIGTERM", value_name = "SIGNAL")]
    pub child_signal: OptionalSignal,
...
/// hack to allow `--child-signal none`.
